...
contract AnvilScript is Script, DeployPermit2, Constants {
    ...
    function run() public {
        ...
        (address hookAddress, bytes32 salt) = HookMiner.find(
            CREATE2_DEPLOYER,
            permissions,
            type(BuybackHook).creationCode,
            abi.encode(address(manager))
        );

        // Deploy the hook using CREATE2.
        vm.broadcast();
        BuybackHook counter = new BuybackHook{salt: salt}(manager);
        require(address(counter) == hookAddress, "AnvilScript: hook address mismatch");
    ...
    function approvePosmCurrency(IPositionManager posm, Currency currency) internal {
        ...
        PERMIT2.approve(
            Currency.unwrap(currency), address(posm), type(uint160).max, type(uint48).max
        );
    }
